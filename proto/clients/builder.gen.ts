...
export const WebRPCSchemaHash = "9190b51b18b867cb6a22af1632fe14e4e29437c3"
...
export interface QuotaControl {
  ...
  verifyUserAuthorization(args: VerifyUserAuthorizationArgs, headers?: object, signal?: AbortSignal): Promise<VerifyUserAuthorizationReturn>
}
...
export interface VerifyUserAuthorizationArgs {
  accessKey: string
  userId: string
}

export interface VerifyUserAuthorizationReturn {
  permission: UserPermission
  serviceAuth: {[key: string]: any}  
}
...
export class QuotaControl implements QuotaControl {
  ...
  verifyUserAuthorization = (args: VerifyUserAuthorizationArgs, headers?: object, signal?: AbortSignal): Promise<VerifyUserAuthorizationReturn> => {
    return this.fetch(
      this.url('VerifyUserAuthorization'),
      createHTTPRequest(args, headers, signal)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          permission: <UserPermission>(_data.permission),
          serviceAuth: <{[key: string]: any}>(_data.serviceAuth),
        }
      })
    }, (error) => {
      throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error.message || ''}` })
