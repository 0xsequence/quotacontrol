// quota-control v0.1.0 9190b51b18b867cb6a22af1632fe14e4e29437c3
// --
...
func WebRPCSchemaHash() string {
	return "9190b51b18b867cb6a22af1632fe14e4e29437c3"
}
...
type QuotaControl interface {
	...
	VerifyUserAuthorization(ctx context.Context, accessKey string, userId string) (*UserPermission, map[string]interface{}, error)
}
...
func (s *quotaControlServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/QuotaControl/UpdateUsage":
		...
		return
	case "/rpc/QuotaControl/VerifyUserAuthorization":
		s.serveVerifyUserAuthorization(ctx, w, r)
		return
	default:
...
func (s *quotaControlServer) serveVerifyUserAuthorization(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveVerifyUserAuthorizationJSON(ctx, w, r)
	default:
		err := ErrorWithCause(ErrWebrpcBadRequest, fmt.Errorf("unexpected Content-Type: %q", r.Header.Get("Content-Type")))
		RespondWithError(w, err)
	}
...
func (s *quotaControlServer) serveVerifyUserAuthorizationJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "VerifyUserAuthorization")
	reqContent := struct {
		Arg0 string `json:"accessKey"`
	...
	func() {
		...
		ret0, ret1, err = s.QuotaControl.VerifyUserAuthorization(ctx, reqContent.Arg0, reqContent.Arg1)
	}()
	respContent := struct {
		Ret0 *UserPermission        `json:"permission"`
		Ret1 map[string]interface{} `json:"serviceAuth"`
	}{ret0, ret1}
...
func NewQuotaControlClient(addr string, client HTTPClient) QuotaControl {
	...
	urls := [15]string{
		...
		prefix + "NotifyEvent",
		prefix + "UpdateUsage",
		prefix + "VerifyUserAuthorization",
	}
...
func (c *quotaControlClient) VerifyUserAuthorization(ctx context.Context, accessKey string, userId string) (*UserPermission, map[string]interface{}, error) {
	in := struct {
		Arg0 string `json:"accessKey"`
		Arg1 string `json:"userId"`
	}{accessKey, userId}
